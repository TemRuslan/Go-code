...
const App = () => {
  ...
  const ContactModal = () => (
    <div className={`fixed inset-0 z-[100] flex items-center justify-center px-6 transition-all duration-500 ${isModalOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
      ...
      <div className="relative bg-white w-full max-w-xl rounded-[3rem] p-10 shadow-2xl animate-in zoom-in-95 duration-300">
        ...
        <h2 className="text-3xl font-black mb-2 uppercase">Начать проект</h2>
        <p className="text-slate-500 mb-8 font-medium">Оставьте заявку, и мы свяжемся с вами в течение дня.</p>
        <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); setIsModalOpen(false); }}>
          <input type="text" placeholder="Ваше имя" className="w-full px-6 py-4 bg-slate-50 rounded-2xl border-none focus:ring-2 focus:ring-slate-200 outline-none font-bold" />
          <input type="email" placeholder="Email" className="w-full px-6 py-4 bg-slate-50 rounded-2xl border-none focus:ring-2 focus:ring-slate-200 outline-none font-bold" />
          <textarea placeholder="Опишите вашу задачу" rows="4" className="w-full px-6 py-4 bg-slate-50 rounded-2xl border-none focus:ring-2 focus:ring-slate-200 outline-none font-bold resize-none" />
  ...
  const Navbar = () => (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${isScrolled || isMobileMenuOpen ? 'bg-white/90 backdrop-blur-xl py-3 shadow-sm' : 'bg-transparent py-6'}`}>
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        <button onClick={() => setActivePage('home')} className="flex items-center gap-3 shrink-0 outline-none">
        ...
        <div className="lg:hidden absolute top-full left-0 right-0 bg-white border-t border-slate-100 p-8 space-y-8 shadow-2xl">
          <div className="grid gap-6">
            <div className="space-y-4">
               ...
               {currentServices.map(s => (
                 <button key={s.id} onClick={() => { setActivePage(s.id); setIsMobileMenuOpen(false); }} className="block text-2xl font-black text-slate-900">{s.title}</button>
               ))}
            </div>
            <div className="h-px bg-slate-100 w-full" />
            <button onClick={() => { setActivePage('projects'); setIsMobileMenuOpen(false); }} className="text-2xl font-black text-slate-900">Проекты</button>
          </div>
        </div>
      )}
    </nav>
  ...
  const ProjectsListPage = () => (
    <div className="max-w-7xl mx-auto px-6 pt-32 pb-20 animate-in fade-in duration-700">
      <div className="mb-16">
        <h1 className="text-4xl sm:text-6xl font-black text-slate-900 mb-4 uppercase tracking-tighter">Наши проекты</h1>
      ...
      <div className="grid md:grid-cols-2 gap-8">
        {currentProjects.map((project) => (
          ...
          >
            <div className="aspect-[16/10] bg-slate-100 rounded-[2rem] mb-6 flex items-center justify-center relative overflow-hidden">
  ...
  const ServiceCards = () => (
    <section className="py-24 px-6 bg-white/50 backdrop-blur-3xl">
  ...
  const Footer = () => (
    <footer className="pt-24 pb-12 px-6 border-t border-slate-100 bg-white/80 backdrop-blur-md">
      <div className="max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-20">
          ...
          <div>
            <h4 className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-8">Услуги</h4>
            <ul className="space-y-4 text-sm font-bold text-slate-600">
              {currentServices.map(s => (
                <li key={s.id}><button onClick={() => setActivePage(s.id)} className="hover:text-slate-900 transition-colors">{s.title}</button></li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-4 text-sm font-bold text-slate-600">
              <li><button onClick={() => setActivePage('projects')} className="hover:text-slate-900">Наши кейсы</button></li>
              <li><button className="hover:text-slate-900">О студии (Плейсхолдер)</button></li>
              <li><button className="hover:text-slate-900">Карьера (Плейсхолдер)</button></li>
              <li><button className="hover:text-slate-900">Блог (Плейсхолдер)</button></li>
            </ul>
          </div>
